...
if __name__ == '__main__':
    while True:
        ...
        if option == '1':
            ...
            if not os.path.isfile('trained_neural_net'):  # check if neural_net file exists
                ...
                run(["C:\\Program Files\\R\\R-4.3.1\\bin\\Rscript.exe", 'getAttributes.r',os.getcwd()], shell=True)  # running R script for extracting data from recorded voice
                ...
                audio_out=trained_neural_net.predict(data)[0]
                print('\nPrediction: \r')
                print('Female' if audio_out == 0 else 'Male')  # print prediction
                break
        elif option == '3':
            print('\nExiting...')
            break
    ...
    while cv2.waitKey(1)<0 :
        ...
        for faceBox in faceBoxes:
            ...
            video_out = 1 if gender == "Male" else 0

            final_gender = round((audio_out*0.5) + (video_out*0.5))    # weighted average

            gender_str = "Female" if final_gender == 0 else "Male"
            ...
            cv2.putText(resultImg, f'{gender_str}, {age}', (faceBox[0], faceBox[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2, cv2.LINE_AA)
